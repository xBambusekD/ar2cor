...
public class ARUWPController : MonoBehaviour {
    ...
    public bool stopRunning() {        
        if (status == ARUWP.ARUWP_STATUS_RUNNING) {
            //if (status == State.WAITING_FOR_FRAME || status == State.DETECTION_RUNNING) {
            bool ret = ARUWP.aruwpStopRunning();
            if (ret) {
                //status = State.BASE_INITIALISED;
                status = ARUWP.ARUWP_STATUS_CTRL_INITIALIZED;
                return true;
            }
            else {
    ...
    public bool shutDownAR() {
        ...
        if (ret) {
            //status = State.NOTHING_INITIALISED;
            status = ARUWP.ARUWP_STATUS_CLEAN;
            return true;
        }
        else {
