...
public class FakeROS : MonoBehaviour {
    ...
    private void CreateFakeInterfaceStateMsg() {
        intefaceStateMsg = new InterfaceStateMsg("ART_BRAIN", 6, new TimeMsg(1520366291, 111136913), 13, 1,
                                new ProgramItemMsg(1, 2, 0, program_type.PICK_FROM_POLYGON, new List<string>(), new List<PoseStampedMsg>(),
                                    new List<PolygonStampedMsg>(), new List<ushort>(), new List<KeyValueMsg>(), new List<SceneLabelMsg>()),
    ...
    private void CreateFakeProgramMsg() {
        programMsg = new ProgramMsg(new ProgramHeaderMsg(13, "Training - polygon", "", false), new List<ProgramBlockMsg>() {
                        new ProgramBlockMsg(1, "Zvedni ze stolu a poloz", new List<ProgramItemMsg> {
                            new ProgramItemMsg(1, 2, 0, program_type.PICK_FROM_POLYGON, new List<string>(){ "ShortLeg" }, new List<PoseStampedMsg>(),
                                ...
                                new List<ushort>(), new List<KeyValueMsg>(), new List<SceneLabelMsg>()),

                            new ProgramItemMsg(2, 3, 0, program_type.PLACE_TO_POSE, new List<string>(), new List<PoseStampedMsg>() {
                                ...
                                new List<PolygonStampedMsg>(), new List<ushort>() { 1 }, new List<KeyValueMsg>(), new List<SceneLabelMsg>()),

                            new ProgramItemMsg(3, 4, 0, program_type.GET_READY, new List<string>(), new List<PoseStampedMsg>(),
                                new List<PolygonStampedMsg>(), new List<ushort>(), new List<KeyValueMsg>(), new List<SceneLabelMsg>()),

                            new ProgramItemMsg(4, 1, 0, program_type.WAIT_UNTIL_USER_FINISHES, new List<string>(), new List<PoseStampedMsg>(),
                                new List<PolygonStampedMsg>(), new List<ushort>() { 2 }, new List<KeyValueMsg>(), new List<SceneLabelMsg>())
                        }, 1, 0)});
    }
