...
namespace HoloToolkit.Unity.UX
{
    ...
    public class BoundingBoxRig : MonoBehaviour
    {
        ...
        [SerializeField]
        private bool useOnlySideRotateHandles = false;
        ...
        public void FocusOnHandle(GameObject handle)
        {
            if (handle != null)
            {
                ...
                if(useResizeHandles) {
                    for (int i = 0; i < resizeHandles.Length; ++i) {
                        resizeHandles[i].SetActive(resizeHandles[i].gameObject == handle);
                    }
                }
            }
            else
            {
                ...
                if(useResizeHandles) {
                    for (int i = 0; i < resizeHandles.Length; ++i) {
                        resizeHandles[i].SetActive(true);
                    }
                }
            }
        }
        ...
        private void UpdateCornerHandles()
        {
            ...
            if (cornerHandles == null)
            {
                ...
                for (int i = 0; i < cornerHandles.Length; ++i)
                {
                    ...
                    //cornerHandles[i].transform.parent = boxInstance.transform;
        ...
        private void UpdateResizeHandles() {
            ...
            if (resizeHandles == null) {
                resizeHandles = new GameObject[6];
                rigScaleGizmoHandles = new BoundingBoxGizmoHandle[6];
                for (int i = 0; i < resizeHandles.Length; ++i) {
                    resizeHandles[i] = GameObject.CreatePrimitive(PrimitiveType.Cube);
                    resizeHandles[i].GetComponent<Renderer>().material = scaleHandleMaterial;
                    resizeHandles[i].transform.localScale = scaleHandleSize;
                    resizeHandles[i].name = "Resize " + i.ToString();
                    //resizeHandles[i].transform.parent = boxInstance.transform;
                    rigScaleGizmoHandles[i] = resizeHandles[i].AddComponent<BoundingBoxGizmoHandle>();
                    rigScaleGizmoHandles[i].Rig = this;
                    rigScaleGizmoHandles[i].ScaleRate = scaleRate;
                    rigScaleGizmoHandles[i].MaxScale = maxScale;
                    rigScaleGizmoHandles[i].TransformToAffect = objectToBound.transform;                    
                    rigScaleGizmoHandles[i].AffineType = BoundingBoxGizmoHandleTransformType.Resize;
                }
            }

            for (int i = 0; i < resizeHandles.Length; ++i) {
                resizeHandles[i].transform.localRotation = objectToBound.transform.rotation;
            }

            resizeHandles[0].transform.localPosition = (handleCentroids[0] + handleCentroids[6]) * 0.5f;
            resizeHandles[1].transform.localPosition = (handleCentroids[4] + handleCentroids[7]) * 0.5f;
            resizeHandles[2].transform.localPosition = (handleCentroids[5] + handleCentroids[3]) * 0.5f;
            resizeHandles[3].transform.localPosition = (handleCentroids[1] + handleCentroids[2]) * 0.5f;
            resizeHandles[4].transform.localPosition = (handleCentroids[6] + handleCentroids[3]) * 0.5f;
            resizeHandles[5].transform.localPosition = (handleCentroids[4] + handleCentroids[1]) * 0.5f;

            rigScaleGizmoHandles[0].Axis = BoundingBoxGizmoHandleAxisToAffect.Z;
            rigScaleGizmoHandles[1].Axis = BoundingBoxGizmoHandleAxisToAffect.X;
            rigScaleGizmoHandles[2].Axis = BoundingBoxGizmoHandleAxisToAffect.Z;
            rigScaleGizmoHandles[3].Axis = BoundingBoxGizmoHandleAxisToAffect.X;
            rigScaleGizmoHandles[4].Axis = BoundingBoxGizmoHandleAxisToAffect.Y;
            rigScaleGizmoHandles[5].Axis = BoundingBoxGizmoHandleAxisToAffect.Y;
        }
        ...
        private void UpdateRotateHandles()
        {
            ...
            if (rotateHandles == null)
            {
                ...
                for (int i = 0; i < rotateHandles.Length; ++i)
                {
                    ...
                    //rotateHandles[i].transform.parent = boxInstance.transform;
        ...
        private void ClearResizeHandles() {
            if (resizeHandles != null) {
                for (int i = 0; i < resizeHandles.Length; ++i) {
                    GameObject.Destroy(resizeHandles[i]);
                }
                resizeHandles = null;
            }
        ...
        private GameObject BuildRig()
        {
            ...
            //rig.transform.parent = boxInstance.transform;
